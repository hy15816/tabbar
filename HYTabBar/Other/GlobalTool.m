...
+(BOOL)isLogin{
    ...
    BOOL loginState = [[hUSERDFS valueForKey:hLOGINSTATE] intValue];
    if (loginState) {
        return YES;
    }
    return NO;
 ...
 */
+(void)loginOut{
    [hUSERDFS setValue:@"" forKey:hDMTCODE];//pzm=nil
    [hUSERDFS setValue:@"0" forKey:hLOGINSTATE];
...
{
    if(![[NSFileManager defaultManager] fileExistsAtPath:hPlistPath(name)])
    {
        NSArray *arr = [[NSArray alloc] init];
        [arr writeToFile:hPlistPath(name) atomically:YES];
    }
}
...
+ (NSMutableArray *)getData:(NSString *)fileName{
    NSMutableArray *array = [[NSMutableArray alloc] initWithContentsOfFile:hPlistPath(fileName)];
    return array;
}
...
+ (void)saveData:(NSString *)carNum carNumId:(NSString *)numId user:(NSString *)name {
    NSMutableArray *array = [GlobalTool getData:hPlistPath(name)];
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    [dict setObject:carNum forKey:@"License_plate_number"];//车牌
    [dict setObject:numId forKey:@"License_plate_id"];//车牌id
    [array addObject:dict];
    [array writeToFile:hPlistPath(name) atomically:YES];
    NSLog(@"path:%@",hPlistPath(name));
}
...
+ (void)updateData:(NSString *)fileName index:(NSInteger)index{
    NSMutableArray *array = [GlobalTool getData:hPlistPath(fileName)];
    ...
    [array writeToFile:hPlistPath(hDFUSERNAME) atomically:YES];
...
+ (void)saveState:(OpenGateType)type forCarNumber:(NSString *)number{
    ...
    //保存开闸状态
    [hUSERDFS setObject:state forKey:[NSString stringWithFormat:@"%@%@",number,@"auto"]];
}
...
+ (OpenGateType)getOpenGateTypeFro:(NSString *)carNum{
    NSInteger state = [[hUSERDFS valueForKey:[NSString stringWithFormat:@"%@%@",carNum,@"auto"]] integerValue];
    if (state == 3) {
