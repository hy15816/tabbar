...
- (void)viewDidLoad {
    ...
    [self createAVCap];
    self.bottomView.backgroundColor = COLORVALUE(0x000000, .2);
...
-(void)addRunImage{
    _lineImgv = [[UIImageView alloc] initWithFrame:CGRectMake((DEVICE_WIDTH- ScanSquareWidth+10)/2, ScanSquareY+20, ScanSquareWidth-10, 2)];
...
-(void)imgvAnimation
{
    if (upOrDown == NO) {
        num ++;
        _lineImgv.frame = CGRectMake((DEVICE_WIDTH- ScanSquareWidth+10)/2, ScanSquareY+20+2*num, ScanSquareWidth-10, 2);
...
-(void)createAVCap{
    ...
    [_capMtdOutput setRectOfInterest:CGRectMake (ScanSquareY/DEVICE_HEIGHT,(( DEVICE_WIDTH - ScanSquareWidth )/2 )/ DEVICE_WIDTH , ScanSquareWidth/DEVICE_HEIGHT , ScanSquareWidth/DEVICE_WIDTH )];//这个属性df-> CGRectMake(0, 0, 1, 1),比例关系，设置时注意，CGRectMake(y, x, h, w),因为是比例，所以 ? = 实际/DEVICE的高或宽
...
-(void)addview:(NSString *)string{
    ...
    shadeView.alpha = .9;
    UILabel *l = [[UILabel alloc] initWithFrame:CGRectMake(0, (DEVICE_HEIGHT-20)/2, DEVICE_WIDTH, 20)];
    l.text = @"正在处理...";
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex) {
        ...
        ScanResultVC *result = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"ScanResultVCIDF"];
